...
export const Add_Node_lesson = async (req, res) => {
  try {
    const {
      title,
      description,
    ...
    const lessonCount = await nodeTutorial.countDocuments();
    ...
    const createdLesson = await nodeTutorial.create(newLesson);
    res.status(201).json({
...
export const GetNodeLesson = async (req, res) => {
  try {
    ...
    // Find the lesson
    const currentLesson = await nodeTutorial.findOne({ order: orderNumber });
