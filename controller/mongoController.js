...
export const Add_Mongo_lesson = async (req, res) => {
  try {
    const {
      title,
      description,
    ...
    const lessonCount = await mongoTutorial.countDocuments();
    ...
    const createdLesson = await mongoTutorial.create(newLesson);
    res.status(201).json({
...
export const GetMongoLesson = async (req, res) => {
  try {
    ...
    // Find the lesson
    const currentLesson = await mongoTutorial.findOne({ order: orderNumber });
