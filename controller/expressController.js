...
export const Add_Express_lesson = async (req, res) => {
  try {
    const {
      title,
      description,
    ...
    const lessonCount = await expressTutorial.countDocuments();
    ...
    const createdLesson = await expressTutorial.create(newLesson);
    res.status(201).json({
...
export const GetExpressLesson = async (req, res) => {
  try {
    ...
    // Find the lesson
    const currentLesson = await expressTutorial.findOne({ order: orderNumber });
